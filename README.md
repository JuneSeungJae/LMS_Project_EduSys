...
### 개발 환경
* **프론트 엔드 작업:** lms_front\app 아래에 작업 폴더를 생성한 뒤 page.tsx를 작성하며 진행합니다.
```bash
cd .\lms_front\
npm install
npm run dev
```
Next.js에서 필요한 모듈을 설치한 뒤 로컬 서버를 실행합니다.

* **백 엔드 작업:** Spring Boot 기반으로 구성되어 있으며, 백엔드 코드는 edusys 디렉터리에서 작업합니다.
```bash
cd .\edusys\
./gradlew bootRun
```
Linux / macOS에서는 `./gradlew bootRun`를 사용하고, Windows 환경에서는 `gradlew.bat bootRun`로 실행하면 됩니다. 빌드 및 테스트는 아래와 같이 진행하면 됩니다.
```bash
./gradlew build
./gradlew test
```
...
1. MySQL Workbench의 접속 정보를 `\LMS_Project_EduSys\edusys\src\main\resources\application.properties`의 MySQL 연결 설정에 기입합니다.
2. 해당 연결에서 아래 쿼리를 실행합니다.
```sql
-- 1. 데이터베이스 생성
